...
public partial class Jan_TwoAgent_Fill_Application
{
    ...
    {
        var gpt3Config = LLMConfiguration.GetLiteLlmConfig();
        ...
        var config = LLMConfiguration.GetLiteLlmConfig();
        var chatAgent = new LiteLlmStreamingAgent(
                "application",
                config,
    ...
    public static async Task<IAgent> CreateAssistantAgent()
    {
        var openaiMessageConnector = new OpenAIChatRequestMessageConnector();
        var config = LLMConfiguration.GetLiteLlmConfig();
        var chatAgent = new LiteLlmAgent("assistant", config, """You create polite prompt to ask user provide missing information""")
            // .RegisterStreamingMiddleware(openaiMessageConnector)
            // .RegisterMiddleware(openaiMessageConnector)
    ...
    public static async Task<IAgent> CreateUserAgent()
    {
        var openaiMessageConnector = new OpenAIChatRequestMessageConnector();
        var config = LLMConfiguration.GetLiteLlmConfig();
        var chatAgent = new LiteLlmAgent(
            "user",
            config,
            """
